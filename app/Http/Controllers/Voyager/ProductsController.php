...
class ProductsController extends VoyagerBaseController
{
    ...
    public function edit (Request $request, $id)
    {
        ...
        $old_modifications = Modification::where('product_id', '=' , $id )->get();
        $categoriesForProduct = $product->categories()->get();

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'categories', 'categoriesForProduct', 'old_modifications'));
    }
    ...
    public function update (Request $request, $id)
    {
        ...
        $request->modification['code'] ? $this->createModification($request,$id) : null;
        $request->deleteModification ? $this->deleteModification($request->deleteModification) : null;
        return redirect()
            ->back()
            ->with([
...
//        $this->updateProductCategories($request, $data->id);
        $request->modification['code'] ? $this->createModification($request,$data->id) : null;
    ...
    protected function createModification($request, $id)
    {
//        dd($request->modification);
        if ($request->modification) {
                Modification::create([
                    'product_id' => $id,
                    'code' => $request->modification['code'],
                    'color' => $request->modification['color'],
                    'size' => $request->modification['size'],
                    'price' => $request->modification['price'],
                    'old_price' => $request->modification['old_price'],
                ]);
        }
    }
    protected function deleteModification($deleteModification)
    {
        foreach ($deleteModification as $modification) {
        Modification::where('id', '=', $modification)->delete();
        }
    }
}
